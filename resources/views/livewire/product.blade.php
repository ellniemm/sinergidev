<div class="container mx-auto p-6 text-black">
    ...
    <div class="bg-gray-50 py-4 px-6 rounded-lg shadow-lg">
        ...
        <form wire:submit.prevent="{{ $updateData ? 'update' : 'store' }}" enctype="multipart/form-data" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    rows="5"
                        ...
                        wire:key="productDescription-{{ $updateData ? $product_id : now() }}"
            ...
            <div>
                ...
                <input type="file" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring focus:ring-indigo-200"
                    wire:model="productImage" accept="image/*"
                ...
                <div id="imagePreview" class="mt-4 border-t-2 border-primary pt-4 relative">
                    @if ($productImage)
                        @if (is_string($productImage))
                            ...
                            <img src="https://sinergi.dev.ybgee.my.id/img/product/{{ $productImage }}"
                                alt="Product Preview" class="max-w-[250px] h-auto object-cover rounded-lg mx-auto">
                        @else
            ...
            <div class="flex space-x-3">
                ...
                @else
                    <button type="submit" class="btn btn-warning text-white" wire:loading.attr="disabled"
                        wire:target="productImage, update">Update</button>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2 space-x-2">
                            <button wire:key="edit-{{ $product['product_id'] }}"
                                wire:click="edit('{{ $product['product_id'] }}')" wire:loading.attr="disabled"
                                class="btn btn-warning text-white">Edit</button>
                            <button wire:key="delete-{{ $product['product_id'] }}"
                                wire:click="delete('{{ $product['product_id'] }}')" wire:loading.attr="disabled"
